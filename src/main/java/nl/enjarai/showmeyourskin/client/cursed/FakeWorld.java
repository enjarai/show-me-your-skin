...
import nl.enjarai.showmeyourskin.mixin.cursed.ReferenceAccessor;
import nl.enjarai.showmeyourskin.mixin.cursed.WorldAccessor;
...
public final class FakeWorld extends World {
    ...
    static {
        ...
        try {
            ...
            accessor.showmeyourskin$setBiomeAccess(new BiomeAccess(worldUnsafe, 1l));
            accessor.showmeyourskin$setBorder(new WorldBorder());
            accessor.showmeyourskin$setDebugWorld(true);
            accessor.showmeyourskin$setProfiler(() -> new ProfilerSystem(() -> 0l, () -> 0, false));
            accessor.showmeyourskin$setProperties(new FakeWorldProperties());
            accessor.showmeyourskin$setRegistryKey(RegistryKey.of(RegistryKeys.WORLD, new Identifier("polymer","fake_world")));
            accessor.showmeyourskin$setDimensionKey(DimensionTypes.OVERWORLD);
            accessor.showmeyourskin$setDimensionEntry(dimType);
            accessor.showmeyourskin$setThread(Thread.currentThread());
            accessor.showmeyourskin$setRandom(Random.create());
            accessor.showmeyourskin$setAsyncRandom(Random.createThreadSafe());
            accessor.showmeyourskin$setBlockEntityTickers(new ArrayList<>());
            accessor.showmeyourskin$setPendingBlockEntityTickers(new ArrayList<>());
            try {
                accessor.showmeyourskin$setDamageSources(new DamageSources(REGISTRY_MANAGER));
            } catch (Throwable e) {

            }

        } catch (Throwable e) {
            ShowMeYourSkin.LOGGER.error("Creating fake world with unsafe failed...", e);
            worldUnsafe = null;
        }
        ...
        try {
            ...
            ShowMeYourSkin.LOGGER.error("Creating fake world in regular way failed...", e);
            worldDefault = null;
        }
